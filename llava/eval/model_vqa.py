...
def eval_model(args):
    ...
    questions = get_chunk(
        questions, args.num_chunks, args.chunk_idx
    )  # no-op with default args

    # Load image data
    image_data = dict(np.load(args.image_data, allow_pickle=True))

    if args.select_layer == -1:  # get shared embeddings
        image_data_key = "transcriptome_embeds"
    elif args.select_layer == -2:  # get image block output (features)
        image_data_key = "transcriptome_features"
    else:
        raise ValueError("select_layer must be -1 or -2")
    nans = np.where(np.isnan(image_data[image_data_key]))
    broken_ids = image_data["orig_ids"][np.unique(nans[0])]
    assert len(broken_ids) == 0, f"Found broken ids: {broken_ids}"

    orig_id_to_int = {
        k: v for k, v in zip(image_data["orig_ids"], range(len(image_data["orig_ids"])))
    }
    ...
    for line in tqdm(questions):
        ...
        image_tensor = torch.tensor(
            image_data[image_data_key][orig_id_to_int[image_id]],
            device=model.device,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="-1 is transcriptome_embeds (CW embedding), -2 is transcriptome features (Geneformer embeddings)",
    )
